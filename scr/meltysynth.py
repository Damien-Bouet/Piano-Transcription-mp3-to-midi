...
class MidiFile:
    ...
    def shift_octave(self, octaves: int = 0, fix_speed=None) -> None:
        """
        Shift the notes in the MIDI file by a specified number of octaves.
        
        :param octaves: Number of octaves to shift (positive for up, negative for down)
        """
        assert 0 <= fix_speed and fix_speed <= 127
        
        shifted_messages = []
        for msg in self._messages:
            # Only modify note-related messages that have note data
            if (msg.type == _MidiMessageType.NORMAL and 
                msg.command in [0x90, 0x80]):  # Note On or Note Off commands
                
                # Calculate new note number, ensuring it stays within MIDI range (0-127)
                
                if msg.data1 + (12 * octaves) - 21 >= 0 and msg.data1 + (12 * octaves) - 21 <= 87:
                    new_note = max(0, min(127, msg.data1 + (12 * octaves)))
                    # Create a new message with the shifted note
                    shifted_msg = _MidiMessage.common2(
                        msg.channel | msg.command, 
                        int(new_note),  # Ensure integer 
                        msg.data2 if fix_speed is None else fix_speed
                    )
                    shifted_messages.append(shifted_msg)
            else:
                # Keep non-note messages as they are
                shifted_messages.append(msg)
        
        # Replace the messages with shifted messages
        self._messages = shifted_messages

    @staticmethod
    def _write_variable_length_quantity(value: int) -> bytes:
        """
        Convert an integer to a variable-length quantity for MIDI file writing.
        
        :param value: Integer to convert
        :return: Bytes representing the variable-length quantity
        """
        result = bytearray()
        while True:
            byte = value & 0x7F
...
def midi_to_wav(midifile, output_file=None, soundfont="../data/grand_piano.sf2", rate=44100, duration=None, progress_bar=True, shift=0, fix_speed=None) -> MidiFileSequencer:
    # Load the SoundFont.
    if output_file is None or ".wav" not in output_file:
        output_file = midifile.replace(".mid", ".wav")
    sound_font = SoundFont.from_file(soundfont)
    
    # Create the synthesizer.
    settings = SynthesizerSettings(rate)
    synthesizer = Synthesizer(sound_font, settings)
    
    # Load the MIDI file.
    midi_file = MidiFile.from_file(midifile)
    
    midi_file.shift_octave(shift, fix_speed)
    # Create the MIDI sequencer.
    sequencer = MidiFileSequencer(synthesizer)
    sequencer.play(midi_file, False)
    
    # The output buffer.
    left = create_buffer(int(settings.sample_rate * midi_file.length))
    right = create_buffer(int(settings.sample_rate * midi_file.length))
    
    if duration is not None:
        left = left[:int(duration*rate)]
        right = right[:int(duration*rate)]
    # Render the waveform.
    sequencer.render(left, right, progress_bar=progress_bar)

    # Save the waveform as a WAV file.
    write_wav_file(settings.sample_rate, left, right, output_file)
